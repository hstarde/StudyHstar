...
{
    KLog(@"setValue %@ forUndefinedKey %@",value,key);
}

- (void)setNilValueForKey:(NSString *)key
{
    KLog(@"setNilValue forKey %@",key);
}
...
- (void)printMothList
{
    ...
    for(int i = 0; i < mothCout_f; i++)
    {
        ...
        KLog(@"方法名：%@,参数个数：%d,编码方式：%@",[NSString stringWithUTF8String:name_s], arguments, [NSString stringWithUTF8String:encoding]);
    }
