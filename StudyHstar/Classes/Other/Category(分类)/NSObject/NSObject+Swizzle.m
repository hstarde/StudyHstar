...
+ (void)overrideInstanceMethod:(SEL)origSelector withInstanceMethod:(SEL)newSelector
{
    ...
    if (!originalMethod) {
        KLog(@"original method %@ not found for class %@", NSStringFromSelector(origSelector), [self class]);
        return;
    }

    Method overrideMethod = class_getInstanceMethod(class, newSelector);
    if (!overrideMethod) {
        KLog(@"original method %@ not found for class %@", NSStringFromSelector(newSelector), [self class]);
        return;
    }
