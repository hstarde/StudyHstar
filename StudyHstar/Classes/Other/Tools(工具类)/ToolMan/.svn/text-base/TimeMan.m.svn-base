...
+(NSString *)getIntTimestamp
{
    ...
    return  [TimeMan getIntTimestamp:dat];
}
//获取 13位时间戳 传入时间
...
+(NSString *)getWeekStr:(NSDate *)time{
    NSInteger weekInt=[TimeMan getWeek:time];
    NSString *weekStr=@"";
...
{
    // 2014-05-01
    NSDate *nowDate = [TimeMan dateWithYMD:[NSDate date]];
    // 2014-04-30
    ...
    if (_arrYear==nil) {
        ...
        for (int i = 1970; i <= now.year; i++) {
            ...
            for (int j = 1; j <= 12; j++) {
                ...
                for (int k=1; k<=[TimeMan calculateDayWithMonth:j andYear:i]; k++) {
